...
async function loadAllWbEntries() {
    ...
    try { const charWorld = ctx.characters?.[ctx.characterId]?.data?.extensions?.world; if (charWorld) worldMap.set(charWorld, '📌 角色绑定'); } catch (_) {}
    try { const chatWorld = ctx.chatMetadata?.world_info; if (chatWorld && !worldMap.has(chatWorld)) worldMap.set(chatWorld, '💬 聊天绑定'); } catch (_) {}
    try { const selWorlds = ctx.selected_world_info || ctx.selectedWorldInfo || []; if (Array.isArray(selWorlds)) { for (const n of selWorlds) { if (n && !worldMap.has(n)) worldMap.set(n, '🌐 全局启用'); } } } catch (_) {}
    try { const wiModule = await import('../../../world-info.js'); const wn = wiModule.world_names; if (Array.isArray(wn)) { for (const n of wn) { if (n && typeof n === 'string' && !worldMap.has(n)) worldMap.set(n, '📚 可用'); } } } catch (e) { console.warn('[MiniMax] world-info.js 导入失败:', e.message); }
    if (worldMap.size === 0) { try { const selectors = ['#world_info select option', '#world_editor_select option','select[name*="world"] option', '.world_info_selector option']; for (const sel of selectors) { document.querySelectorAll(sel).forEach(opt => { const n = (opt.value || opt.textContent || '').trim(); if (n && n !== '' && n.toLowerCase() !== 'none' && !worldMap.has(n)) worldMap.set(n, '📚 可用'); }); } } catch (_) {} }
    if (worldMap.size === 0) { try { const res = await fetch('/api/worldinfo/settings', { method: 'POST', headers: { ...getRequestHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({}) }); if (res.ok) { const data = await res.json(); const names = data.world_names || data.worldNames || []; for (const n of names) { if (n && !worldMap.has(n)) worldMap.set(n, '📚 可用'); } } } catch (_) {} }
    ...
    for (const [worldName, source] of worldMap) {
        try {
            ...
            if (!data || !data.entries) { try { const res = await fetch('/api/worldinfo/get', { method: 'POST', headers: { ...getRequestHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ name: worldName }) }); if (res.ok) data = await res.json(); } catch (_) {} }
            ...
            for (const entry of Object.values(data.entries)) {
                ...
                allEntries.push({ key: worldName + '::' + entry.uid, title: entry.comment || ('条目 ' + entry.uid), content: entry.content || '', worldName, source, uid: entry.uid, disabled: !!entry.disable, keys });
            }
        } catch (e) { console.warn('[MiniMax] 加载世界书失败:', e.message); }
    }
    return allEntries;
}
...
function applyPreProcessRules(text) {
    ...
    for (const rule of rules) {
        ...
        if (rule.mode === 'extract') {
            const matches = []; let m;
            while ((m = re.exec(text)) !== null) { const t = (m[1] !== undefined ? m[1] : m[0]).trim(); if (t) matches.push(t); }
...
function getBuiltinQuoteRule() {
    return { id: 'builtin-quotes', enabled: true, name: '引号内容', pattern: '[\\u0022\\u201c\\u300c\\u300e\\u2018]([^\\u0022\\u201c\\u201d\\u300c\\u300d\\u300e\\u300f\\u2018\\u2019]{1,500}?)[\\u0022\\u201d\\u300d\\u300f\\u2019]', flags: 'g', mode: 'extract' };
}
...
function loadSettings() {
    ...
    if (!settings.llmPresets.length && (settings.formatterApiUrl || settings.vcLlmApiUrl)) {
        ...
        if (url || model) { settings.llmPresets.push({ name: '迁移预设', url, key, format: settings.formatterFormat || API_FORMATS.OAI, model }); settings.formatterPresetIdx = 0; }
    }
    if (!Array.isArray(settings.regexRules) || settings.regexRules.length === 0) {
        settings.regexRules = [getBuiltinQuoteRule()];if (settings.customRegexPattern && settings.regexMode === 'custom') {
            settings.regexRules.push({ id: 'migrated-' + Date.now(), enabled: true, name: '迁移正则', pattern: settings.customRegexPattern, flags: settings.customRegexFlags || 'g', mode: 'extract' });
        }
    }
    delete settings.quoteOnly; delete settings.regexMode; delete settings.customRegexPattern; delete settings.customRegexFlags;
    settings._loaded = true;
}

function simpleHash(t) { if (!t) return 0; let h = 0; for (let i = 0; i < t.length; i++) h = ((h << 5) - h) + t.charCodeAt(i), h |= 0; return Math.abs(h); }
function buildMessageKey(ctx, id, m) { return `${ctx.chatId ||'no-chat'}:${id}:${m?.swipe_id || 0}:${simpleHash(m?.mes)}`; }
function getMessageData(id) { const ctx = getContext(); const m = ctx?.chat?.[id]; return { ctx, message: m, key: m ? buildMessageKey(ctx, id, m) : '' }; }
...
async function uploadToSTServer(blob, filename) {
    try {
        ...
        const res = await fetch('/api/files/upload', { method: 'POST', headers: { ...getRequestHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ name: `minimax_${filename}`, data: base64 }) });
...
async function syncToSTSecrets(apiKey, groupId) {
    ...
    } catch (e) { console.warn('[MiniMax] secrets同步失败:', e.message); }
}

async function proxyFetch(url, options = {}) {
    const res = await fetch(url, { method: options.method || 'GET', headers: {'Content-Type': 'application/json', ...(options.headers || {}) }, body: options.body != null ? JSON.stringify(options.body) : undefined });
...
//════════════════════════════════════════
//  TTS 核心：直连 MiniMax / 中转站
// ════════════════════════════════════════

function getAudioFieldFromResponse(data) {
    return data.data?.audio_url || data.audio_url || data.data?.url || data.url|| data.data?.audio_file || data.audio_file || data.data?.file_url || data.file_url
        || data.data?.audio || data.audio || data.audio_data || data.data?.audio_data;
}

async function extractAudioFromResponse(data, fmt) {
    const audioUrl = data.data?.audio_url || data.audio_url || data.data?.url || data.url || data.data?.audio_file || data.audio_file || data.data?.file_url || data.file_url;
    if (audioUrl && typeof audioUrl === 'string' && (audioUrl.startsWith('http') || audioUrl.startsWith('//'))) {
        console.log('[MiniMax TTS] 下载音频 URL:', audioUrl);
        const audioRes = await fetch(audioUrl.startsWith('//') ? 'https:' + audioUrl : audioUrl);
        if (!audioRes.ok) throw new Error(`音频下载失败: HTTP ${audioRes.status}`);
        return await audioRes.blob();
    }
    const audioData = data.data?.audio || data.audio || data.audio_data || data.data?.audio_data;
    if (audioData && typeof audioData === 'string') {
        ...
        return new Blob([byteArray], { type: mimeMap[fmt] || 'audio/mpeg' });
    }
    const hexData = data.data?.audio_hex || data.audio_hex;
    if (hexData && typeof hexData === 'string') {
        const bytes = new Uint8Array(hexData.match(/.{1,2}/g).map(byte => parseInt(byte, 16)));
        return new Blob([bytes], { type: 'audio/mpeg' });
    }
    console.error('[MiniMax TTS] 未知响应格式:', JSON.stringify(data).slice(0, 500));
...
async function pollTaskResult(baseUrl, headers, taskId) {
    ...
    const pollUrls = [
        ...
        baseUrl.replace(/\/tts\/?$/, `/tasks/${taskId}`),
        baseUrl.replace(/\/tts\/?$/, `/task/${taskId}`),
        baseUrl.replace(/\/tts\/?$/, `/result/${taskId}`),
    ];
    for (let attempt = 0; attempt < MAX_POLLS; attempt++) {
        await new Promise(r => setTimeout(r, POLL_INTERVAL));
        for (const pollUrl of (attempt === 0 ? pollUrls : [pollUrls[0]])) {
            try {
                let res = await fetch(pollUrl, { method: 'GET', headers });
                if (res.status === 405|| res.status === 404) {
                    res = await fetch(baseUrl, { method: 'POST', headers, body: JSON.stringify({ task_id: taskId }) });
                }
                if (!res.ok) continue;
                const ct = (res.headers.get('content-type') || '').toLowerCase();
                if (ct.includes('audio/') || ct.includes('octet-stream')) return await res.blob();
                const data = await res.json();
                console.log(`[MiniMax TTS] 轮询 #${attempt + 1}:`, JSON.stringify(data).slice(0, 300));
                const status = (data.status || data.data?.status || '').toLowerCase();
                if (status === 'error' || status === 'failed') throw new Error(data.message || data.error || '任务失败');
                if (status === 'completed' || status === 'done' || status === 'success' || status === 'finished') return await extractAudioFromResponse(data, 'mp3');
                if (getAudioFieldFromResponse(data)) return await extractAudioFromResponse(data, 'mp3');if (attempt === 0) { pollUrls.length = 0; pollUrls.push(pollUrl); }
                break;
            } catch (e) {
    ...
    throw new Error('任务超时（等待超过2分钟），请检查中转站状态');
}
...
async function directMinimaxTts(text, options) {
    ...
    if (apiHost === 'custom') {
        ...
        if (!customHost) throw new Error('请在「中转地址」中填写完整的中转站URL');
        url = customHost;
        isRelay = true;
    } else {
    ...
    const body = {
        model, text, stream: false, output_format: 'url',
    ...
    console.log('[MiniMax TTS] 请求:', url, 'model:', model, 'voice:', voiceId, 'relay:', isRelay);
    ...
    if (!res.ok) {
        ...
        try { const e = await res.json(); msg = e.base_resp?.status_msg || e.error?.message || (typeof e.error === 'string' ? e.error : null) || e.message || e.detail || (e.code ? `[${e.code}]` : null) || JSON.stringify(e).slice(0, 200) || msg; } catch (_) { try { msg = await res.text(); } catch (_) {} }
        throw new Error(msg);
    }
    ...
    const data = await res.json();
    console.log('[MiniMax TTS] 响应:', JSON.stringify(data).slice(0, 500));

    if (data.status === 'error' || (data.base_resp?.status_code !== 0 && data.base_resp?.status_code !== undefined)) {
        throw new Error(data.base_resp?.status_msg || (typeof data.error === 'string' ? data.error : data.error?.message) || data.message || `[${data.code || 'ERROR'}]`);
    }

    const taskId = data.task_id || data.data?.task_id || data.id || data.data?.id;
    if (taskId && !getAudioFieldFromResponse(data)) {
        console.log('[MiniMax TTS] 异步任务模式, task_id:', taskId);
...
async function getAudioBlob(item) {
    ...
    try {
        const res = await fetch(PROXY_ENDPOINT, {
            ...
            body: JSON.stringify({ text: item.text, apiHost: s().apiHost, model: item.options.model, voiceId: item.options.voiceId, speed: item.options.speed, volume: item.options.vol, pitch: item.options.pitch, format: item.options.audioFormat, emotion: item.options.emotion, language: item.options.language || undefined }),
        });
        if (res.status === 404|| res.status === 501|| res.status === 405) throw new Error('PROXY_NOT_AVAILABLE');
        if (!res.ok) { let msg = `HTTP ${res.status}`; try { const e = await res.json(); msg = e.error || e.message || msg; } catch (_) {} throw new Error(msg); }
        blob = await res.blob();
    } catch (proxyErr) {
...
async function formatWithSecondaryApi(m) {
    ...
    if (format === API_FORMATS.OAI) {
        ...
        const data = await proxyFetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(preset.key ? { 'Authorization': `Bearer ${preset.key.trim()}` } : {}) }, body: { model: preset.model, messages: [{ role: 'system', content: prompt }, { role: 'user', content: applyPreProcessRules(m.mes) }], temperature: 0.1} });
        text = data.choices?.[0]?.message?.content;
    } else {
        const baseUrl = (preset.url || '').trim().replace(/\/+$/, '');
        const gUrl = `${baseUrl}/v1beta/models/${preset.model}:generateContent`;
        const data = await proxyFetch(gUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(preset.key ? { 'x-goog-api-key': preset.key.trim() } : {}) }, body: { contents: [{ role: 'user', parts: [{ text: `System Prompt: ${prompt}\n\nUser Message: ${applyPreProcessRules(m.mes)}` }] }] } });
...
function runRegexExtraction(cleanText, rules, speakerName) {
    let extracted = [];
    for (const rule of rules.filter(r => r.mode === 'extract')) {
        const flags = (rule.flags || 'g').includes('g') ? (rule.flags || 'g') : (rule.flags || 'g') + 'g';
        let re; try { re = new RegExp(parsePattern(rule.pattern), flags); } catch (e) { continue; }
        let qm;
        while ((qm = re.exec(cleanText)) !== null) {
            const inner = (qm[1]?.replace(/\n+/g, ' ').trim()) || '';
            const full = (qm[0]?.replace(/\n+/g, ' ').trim()) || '';
            if (inner && inner.length >= 2) {
                extracted.push({ text: inner, fullMatch: full, speaker: speakerName });
            }
        }
    }
...
async function generateMessageSpeech(id, forced = false) {
    ...
    try {
        ...
        if (s().formatterEnabled) {
            ...
            raw = rules.length ? runRegexExtraction(cleanText, rules, message.name) : [];
        }
...
function extractSegmentsOnly(id) {
    ...
    const extracted = runRegexExtraction(cleanText, rules, message.name);
...
function refreshAllMessageButtons() {
    document.querySelectorAll('#chat .mes[mesid]').forEach(el => {
        ...
        } else { ready = !!(h?.versions?.length > 0); }
        btn.classList.toggle('ready', ready);});
}

function makeBubble(mesid, segidx, item, withText = false) {
    const el = document.createElement('span');
    el.className = 'mm-bubble'; el.dataset.mesid = mesid; el.dataset.segidx = segidx; el.title = item.text;
...
function injectAfterText(container, searchText, insertEl) {
    ...
    const filter = { acceptNode: n => n.parentElement.closest('.mm-bubble, .mm-bubble-strip') ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT };
...
function injectBubbles(mesid) {
    ...
    if (isQuoteMode) {
        // 引号模式：用完整匹配（含引号）精确定位，找不到就不显示
        items.forEach(({ it: item, idx }) => {
            const bubble = makeBubble(mesid, idx, item, false);
            const searchText = item.fullMatch || '';
            if (searchText) {
                injectAfterText(textEl, searchText, bubble);
            }
            // 找不到就丢弃，不显示底部条
        });
    } else {
        ...
        if (unmatched.length) {
            const strip = document.createElement('div'); strip.className = 'mm-bubble-strip';
...
function removeAllBubbles() {
    document.querySelectorAll('.mm-bubble, .mm-bubble-strip').forEach(el => el.remove());document.querySelectorAll('[data-mm-bub-ver]').forEach(el => el.removeAttribute('data-mm-bub-ver'));
}
...
function openParamsEditor(id) {
    ...
    const render = () => {
        ...
        const html = `<div id="minimax_quote_tts_editor" class="minimax-tts-editor-mask"><div class="minimax-tts-editor-dialog">
            <div class="minimax-tts-editor-header">
                ...
                <div style="display:flex;gap:10px;align-items:center;flex:1">
                    ...
                    <button class="menu_button v-next" style="width:40px;">&gt;</button><button class="menu_button v-del" style="color:#ef5350;">删除</button>
                </div>
        ...
        $('#minimax_quote_tts_editor .v-del').on('click', () => {
            if (h.versions.length <= 1) { delete s().serverHistory[key]; saveSettingsDebounced(); refreshAllMessageButtons(); $('#minimax_quote_tts_editor').remove(); return; }
            h.versions.splice(h.activeIndex, 1); h.activeIndex = Math.min(h.activeIndex, h.versions.length - 1); saveSettingsDebounced(); refreshAllMessageButtons(); render();
        });
        const invalidateBubbles = () => { const te = document.querySelector(`#chat .mes[mesid="${id}"] .mes_text`); if (te) te.removeAttribute('data-mm-bub-ver'); setTimeout(() => { injectBubbles(id); refreshBubbleStates(id); refreshAllMessageButtons(); }, 100); };
...
function refreshAllLlmPresetSelects() {
    ...
    document.querySelectorAll('.llm-preset-sel').forEach(el => { const cur = el.value; el.innerHTML = opts; if (Number(cur) >= 0 && Number(cur) < presets.length) el.value = cur; });
}
...
function injectStyles() {
    ...
    style.textContent = `
.mm-config-mask{display:none;position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.55);backdrop-filter:blur(4px);justify-content:center;align-items:center;padding:16px}
.mm-config-mask.mm-config-open{display:flex!important}
.mm-config-dialog{width:min(680px,96vw);max-height:92vh;background:var(--SmartThemeBlurTintColor,#1a1c2a);color:var(--SmartThemeBodyColor,#ccc);border-radius:16px;box-shadow:0 12px 48px rgba(0,0,0,.45);display:flex;flex-direction:column;overflow:hidden}
.mm-config-header{display:flex;align-items:center;padding:10px 16px;border-bottom:1px solid rgba(255,255,255,.08);flex-shrink:0;gap:8px}
.mm-config-close{background:none;border:none;color:inherit;font-size:1.2rem;cursor:pointer;padding:4px 8px;opacity:.6;flex-shrink:0}.mm-config-close:hover{opacity:1}
.mm-config-body{flex:1;overflow-y:auto;padding:16px}
.mm-tab-bar{display:flex;gap:2px;flex:1;flex-wrap:wrap}
.mm-tab{background:transparent;border:none;color:inherit;padding:6px 14px;cursor:pointer;border-radius:8px 8px 0 0;font-size:.88rem;opacity:.55;white-space:nowrap}.mm-tab:hover{opacity:.8;background:rgba(255,255,255,.04)}.mm-tab.active{opacity:1;background:rgba(255,255,255,.08);font-weight:600}
.mm-tab-panel{display:none}.mm-tab-panel.active{display:block}
.mm-config-dialog .mm-row{display:flex;align-items:center;gap:8px;margin-bottom:10px}
.mm-config-dialog .mm-row>label{min-width:85px;flex-shrink:0;font-size:.88rem;opacity:.8}
.mm-config-dialog .text_pole{flex:1;min-width:0;max-width:100%;box-sizing:border-box;height:34px!important;font-size:.88rem!important}
...
.mm-section-title{font-weight:600;font-size:.95rem;margin:16px 0 8px;display:flex;align-items:center;gap:10px}
.mm-desc{font-size:.82rem;opacity:.55;margin:0 0 10px;line-height:1.4}
.mm-hint{font-size:.8rem;opacity:.5;margin:4px 0 10px}
.mm-inline-hint{font-size:.78rem;opacity:.5;white-space:nowrap}
.mm-voice-lib-row{display:flex;gap:6px;margin-bottom:6px;align-items:center}
.mm-binding-row{display:flex;gap:6px;margin-bottom:6px;align-items:center;flex-wrap:wrap}.mm-binding-row .text_pole{flex:1;min-width:80px}
.mm-rule-header-row{display:flex;gap:6px;font-size:.78rem;opacity:.5;padding:0 0 4px}.mm-rule-header-row>span{flex:1}.mm-rule-header-row>.mm-rule-toggle{flex:0 0 20px}.mm-rule-header-row>.mm-rule-mode{flex:0 0 68px}.mm-rule-header-row>.mm-rule-del{flex:0 0 30px}
.mm-rule-row{display:flex;gap:6px;margin-bottom:6px;align-items:center}.mm-rule-row .text_pole{flex:1;min-width:0}.mm-rule-row .mm-rule-toggle{flex:0 0 20px}
.mes_quote_tts{cursor:pointer;opacity:.5}.mes_quote_tts:hover{opacity:.85}.mes_quote_tts.ready{opacity:1;color:#4caf50}
.mm-bubble{display:inline-flex;align-items:center;gap:3px;padding:1px 6px;margin:0 2px;border-radius:10px;background:rgba(110,160,255,.12);cursor:pointer;font-size:.78rem;vertical-align:middle;transition:background .15s;white-space:nowrap}.mm-bubble:hover{background:rgba(110,160,255,.25)}.mm-bubble i{font-size:.7rem}
.mm-bubble-cached{background:rgba(76,175,80,.15)}.mm-bubble-cached:hover{background:rgba(76,175,80,.3)}
.mm-bubble-loading{opacity:.5;pointer-events:none}.mm-bubble-playing{background:rgba(255,165,0,.2)}
.mm-bubble-strip{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;padding-top:6px;border-top:1px dashed rgba(255,255,255,.1)}
.minimax-tts-editor-mask{position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,.6);display:flex;justify-content:center;align-items:center;padding:16px}
.minimax-tts-editor-dialog{width:min(600px,94vw);max-height:88vh;background:var(--SmartThemeBlurTintColor,#1a1c2a);color:var(--SmartThemeBodyColor,#ccc);border-radius:16px;display:flex;flex-direction:column;overflow:hidden}
.minimax-tts-editor-header{display:flex;align-items:center;padding:12px 16px;border-bottom:1px solid rgba(255,255,255,.08);gap:8px}
.minimax-tts-editor-body{flex:1;overflow-y:auto;padding:16px}
.minimax-tts-editor-item{margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,.05)}
.minimax-tts-editor-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
.minimax-tts-editor-row-flex{display:flex;align-items:center;gap:6px}.minimax-tts-editor-row-flex label{min-width:36px;font-size:.82rem;opacity:.7}
.minimax-tts-editor-actions{display:flex;gap:10px;padding:12px 16px;border-top:1px solid rgba(255,255,255,.08)}.minimax-tts-editor-actions .menu_button{flex:1}
`;
...
function renderVoiceLibrary() {
    const container = document.getElementById('mm_voice_lib_rows'); if (!container) return; container.innerHTML = '';
    (s().voiceLibrary || []).forEach((v, i) => {
        const el = document.createElement('div'); el.className = 'mm-voice-lib-row';
        ...
        el.querySelector('.vl-del').addEventListener('click', () => { s().voiceLibrary.splice(i, 1); saveSettingsDebounced(); renderVoiceLibrary(); refreshVoiceSelects(); });
...
function refreshVoiceSelects() {
    ...
    document.querySelectorAll('.mm-voice-sel').forEach(el => { const cur = el.value; el.innerHTML = opts; if (cur && [...el.options].some(o => o.value === cur)) el.value = cur; });
}

function renderRules() {
    const container = document.getElementById('mm_rule_rows'); if (!container) return; container.innerHTML = '';
    (s().regexRules || []).forEach((rule, i) => {
        const el = document.createElement('div'); el.className = 'mm-rule-row';
        el.innerHTML = `<input type="checkbox" class="mm-rule-toggle" ${rule.enabled ? 'checked' : ''}><input class="text_pole mm-rule-name" placeholder="规则名" value="${escHtml(rule.name || '')}"><input class="text_pole mm-rule-pattern" placeholder="正则" value="${escHtml(rule.pattern || '')}"><select class="text_pole mm-rule-mode" style="max-width:68px"><option value="extract" ${rule.mode === 'extract' ? 'selected' : ''}>提取</option><option value="exclude" ${rule.mode === 'exclude' ? 'selected' : ''}>排除</option></select><button class="menu_button mm-rule-del">×</button>`;
        ...
        el.querySelector('.mm-rule-del').addEventListener('click', () => { s().regexRules.splice(i, 1); saveSettingsDebounced(); renderRules(); });
        container.appendChild(el);
    });
}

function renderPreProcessRules() {
    const container = document.getElementById('mm_pre_rule_rows'); if (!container) return; container.innerHTML = '';
    (s().llmPreProcessRules || []).forEach((rule, i) => {
        const el = document.createElement('div'); el.className = 'mm-rule-row';
        el.innerHTML = `<input type="checkbox" class="mm-rule-toggle" ${rule.enabled ? 'checked' : ''}><input class="text_pole mm-rule-name" placeholder="规则名" value="${escHtml(rule.name || '')}"><input class="text_pole mm-rule-pattern" placeholder="正则" value="${escHtml(rule.pattern || '')}"><select class="text_pole mm-rule-mode" style="max-width:68px"><option value="extract" ${rule.mode === 'extract' ? 'selected' : ''}>提取</option><option value="exclude" ${rule.mode === 'exclude' ? 'selected' : ''}>排除</option></select><button class="menu_button mm-rule-del">×</button>`;
        ...
        el.querySelector('.mm-rule-del').addEventListener('click', () => { s().llmPreProcessRules.splice(i, 1); saveSettingsDebounced(); renderPreProcessRules(); });
        container.appendChild(el);
    });
}
...
function renderBindings() {
    ...
    const container = $('#mm_b_rows'); if (!container.length) return; container.empty();
    const lib = s().voiceLibrary || [];
    s().characterBindingsMap[id].forEach((b, i) => {
        ...
        const row = $(`<div class="mm-binding-row"><select class="text_pole b-type"><option value="${TARGET_TYPE.CURRENT_CHARACTER}">${escHtml(c.name2|| '角色')}</option><option value="${TARGET_TYPE.CURRENT_USER}">${escHtml(c.name1 || '你')}</option><option value="${TARGET_TYPE.CUSTOM}">自定义</option></select><input class="text_pole b-name" placeholder="名称" value="${escHtml(b.customName || '')}" style="${b.targetType === TARGET_TYPE.CUSTOM ? '' : 'display:none'}"><select class="text_pole b-model">${MODEL_OPTIONS.map(o => `<option value="${o.value}">${o.label}</option>`).join('')}</select><select class="text_pole b-voice-lib mm-voice-sel">${voiceOpts}</select><input class="text_pole b-voice" placeholder="voiceId" value="${escHtml(libMatch ? '' : b.voiceId || '')}" style="${libMatch ? 'display:none' : ''}"><button class="menu_button b-del" style="padding:4px 10px;flex-shrink:0">×</button></div>`);
        ...
        row.find('.b-voice-lib').val(libMatch ? b.voiceId : '').on('change', function () { const v = $(this).val(); b.voiceId = v; row.find('.b-voice').toggle(!v).val(''); saveSettingsDebounced(); });
...
function renderRegexPresets() {
    const sel = document.getElementById('mm_regex_presets'); if (!sel) return;
    sel.innerHTML = '<option value="-1">-- 选择预设 --</option>';
    (s().regexPresets || []).forEach((p, i) => { const opt = document.createElement('option'); opt.value = i; opt.textContent = p.name; sel.appendChild(opt); });
}
...
function populateConfigFields() {
    ...
    const voiceSel = el('mm_voice_sel'), voiceInput = el('mm_voice');
...
<p class="mm-desc">为当前角色卡的各角色分配专属音色。</p>
<div id="mm_b_rows"></div>
</div>
<div class="mm-tab-panel" data-panel="llm">
<p class="mm-desc">管理用于格式化的 LLM API 预设。</p>
...
<p class="mm-desc">正则规则手动提取朗读内容；或启用副LLM交由AI结构化处理。</p>
...
<div class="mm-rule-header-row"><span class="mm-rule-toggle"></span><span>名称</span><span>正则表达式</span><span class="mm-rule-mode">模式</span><span class="mm-rule-del"></span></div>
...
<div class="mm-section-title" style="margin-top:16px">副LLM 格式化</div>
...
<p class="mm-desc">发送给副LLM之前，对消息文本执行正则处理。</p>
<div class="mm-rule-header-row"><span class="mm-rule-toggle"></span><span>名称</span><span>正则表达式</span><span class="mm-rule-mode">模式</span><span class="mm-rule-del"></span></div>
<div id="mm_pre_rule_rows"></div>
</div>
</div></div></div>`;document.body.insertAdjacentHTML('beforeend', panelHtml);
        ...
        document.getElementById('mm-config-mask').addEventListener('click', function (e) { if (e.target === this) this.classList.remove('mm-config-open'); });
        document.querySelector('#mm-config-dialog .mm-config-close').addEventListener('click', () => { document.getElementById('mm-config-mask').classList.remove('mm-config-open'); });
        ...
        document.getElementById('mm_apihost').addEventListener('change', function () { const row = document.getElementById('mm_custom_host_row'); if (row) row.style.display = this.value === 'custom' ? '' : 'none'; });
        document.getElementById('mm_show_bubbles').addEventListener('change', function () { s().showBubbles = this.checked; saveSettingsDebounced(); if (this.checked) refreshAllBubbles(); else removeAllBubbles(); });
        ['mm_key', 'mm_gid', 'mm_apihost', 'mm_custom_host', 'mm_model', 'mm_voice_sel', 'mm_voice', 'mm_speed', 'mm_vol', 'mm_tts_lang', 'mm_autoplay'].forEach(id => { const el = document.getElementById(id); if (!el) return; el.addEventListener('input', syncTts); el.addEventListener('change', syncTts); });
        document.getElementById('mm_voice_sel').addEventListener('change', function () { document.getElementById('mm_voice').style.display = this.value ?'none' : ''; });

        const TEST_TEXTS = { '': '你好，我是MiniMax语音。', zh: '你好，我是MiniMax语音。', en: 'Hello, I am MiniMax voice.', ja: 'こんにちは、MiniMaxの音声です。', ko: '안녕하세요, 저는 MiniMax 음성입니다.', fr: 'Bonjour, je suis la voix MiniMax.', de: 'Hallo, ich bin die MiniMax-Stimme.', es: 'Hola, soy la voz MiniMax.', pt: 'Olá, sou a voz MiniMax.', id: 'Halo, saya adalah suara MiniMax.', ar: 'مرحبًا، أنا صوت MiniMax.' };
        document.getElementById('mm_test_tts').addEventListener('click', async () => {
            try { const lang = s().ttsLanguage || ''; const text = TEST_TEXTS[lang] || TEST_TEXTS['']; const item = { text, options: buildSynthesisOptions(null, null), serverPath: null }; const blob = await getAudioBlob(item); new Audio(URL.createObjectURL(blob)).play(); toastr.success('语音连通成功！'); }
            catch (e) { toastr.error('语音测试失败: ' + e.message); }
        });
        document.getElementById('mm_add_voice').addEventListener('click', () => { if (!s().voiceLibrary) s().voiceLibrary = []; s().voiceLibrary.push({ name: '', voiceId: '' }); renderVoiceLibrary(); saveSettingsDebounced(); });
        ...
        document.getElementById('mm_add_rule').addEventListener('click', () => { if (!s().regexRules) s().regexRules = []; s().regexRules.push({ id: 'rule-' + Date.now(), enabled: true, name: '新规则', pattern: '', flags: 'g', mode: 'extract' }); renderRules(); saveSettingsDebounced(); });
        document.getElementById('mm_add_pre_rule').addEventListener('click', () => { if (!s().llmPreProcessRules) s().llmPreProcessRules = []; s().llmPreProcessRules.push({ id: 'pre-' + Date.now(), enabled: true, name: '新规则', pattern: '', mode: 'exclude' }); renderPreProcessRules(); saveSettingsDebounced(); });
        document.getElementById('mm_regex_save_p').addEventListener('click', () => { const name = prompt('预设名称：'); if (!name) return; if (!s().regexPresets) s().regexPresets = []; s().regexPresets.push({ name, rules: JSON.parse(JSON.stringify(s().regexRules || [])) }); renderRegexPresets(); saveSettingsDebounced(); toastr.success('已保存'); });
        document.getElementById('mm_regex_upd_p').addEventListener('click', () => { const i = Number(document.getElementById('mm_regex_presets').value); if (i < 0 || !s().regexPresets?.[i]) return; s().regexPresets[i].rules = JSON.parse(JSON.stringify(s().regexRules || [])); saveSettingsDebounced(); toastr.success('已更新'); });
        document.getElementById('mm_regex_presets').addEventListener('change', function () { const i = Number(this.value); if (i < 0 || !s().regexPresets?.[i]) return; s().regexRules = JSON.parse(JSON.stringify(s().regexPresets[i].rules)); renderRules(); saveSettingsDebounced(); });
        document.getElementById('mm_regex_del_p').addEventListener('click', () => { const i = Number(document.getElementById('mm_regex_presets').value); if (i < 0 || !s().regexPresets?.[i]) return; s().regexPresets.splice(i, 1); renderRegexPresets(); saveSettingsDebounced(); });
        document.getElementById('mm_regex_export').addEventListener('click', () => { const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([JSON.stringify(s().regexPresets || [], null, 2)], { type: 'application/json' })); a.download = 'regex_presets.json'; a.click(); });
        document.getElementById('mm_regex_import').addEventListener('click', () => { const inp = document.createElement('input'); inp.type = 'file'; inp.accept = '.json'; inp.onchange = async () => { try { const data = JSON.parse(await inp.files[0].text()); if (Array.isArray(data)) { if (!s().regexPresets) s().regexPresets = []; s().regexPresets.push(...data); renderRegexPresets(); saveSettingsDebounced(); toastr.success(`已导入 ${data.length} 个预设`); } } catch (e) { toastr.error('导入失败: ' + e.message); } }; inp.click(); });

        const syncFormatter = () => { s().formatterEnabled = document.getElementById('mm_f_en').checked; s().formatterPresetIdx = Number(document.getElementById('mm_f_preset_sel').value); s().formatterSystemPrompt = document.getElementById('mm_f_prompt').value; saveSettingsDebounced(); };
        const selFT = document.getElementById('mm_f_templates');
        const upFTemplates = () => { selFT.innerHTML = '<option value="-1">-- 新建模板 --</option>'; (s().formatterTemplates || []).forEach((t, i) => { const o = document.createElement('option'); o.value = i; o.textContent = t.name; selFT.appendChild(o); }); };
        ['mm_f_en', 'mm_f_preset_sel', 'mm_f_prompt'].forEach(id => { const el = document.getElementById(id); if (!el) return; el.addEventListener('input', syncFormatter); el.addEventListener('change', syncFormatter); });
        selFT.addEventListener('change', function () { const t = (s().formatterTemplates || [])[Number(this.value)]; if (t) { document.getElementById('mm_f_prompt').value = t.content; syncFormatter(); } });
        document.getElementById('mm_f_save_t').addEventListener('click', () => { const n = prompt('模板名:'); if (!n) return; if (!s().formatterTemplates) s().formatterTemplates = []; s().formatterTemplates.push({ name: n, content: document.getElementById('mm_f_prompt').value }); upFTemplates(); saveSettingsDebounced(); });
        document.getElementById('mm_f_upd_t').addEventListener('click', () => { const i = Number(selFT.value); if (i >= 0) { (s().formatterTemplates || [])[i].content = document.getElementById('mm_f_prompt').value; toastr.success('更新成功'); saveSettingsDebounced(); } });
        document.getElementById('mm_f_del_t').addEventListener('click', () => { const i = Number(selFT.value); if (i >= 0) { (s().formatterTemplates || []).splice(i, 1); upFTemplates(); saveSettingsDebounced(); } });
        document.getElementById('mm_f_export_t').addEventListener('click', () => { const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([JSON.stringify(s().formatterTemplates || [], null, 2)], { type: 'application/json' })); a.download = 'formatter_templates.json'; a.click(); });
        document.getElementById('mm_f_import_t').addEventListener('click', () => { const inp = document.createElement('input'); inp.type = 'file'; inp.accept = '.json'; inp.onchange = async () => { try { const data = JSON.parse(await inp.files[0].text()); if (Array.isArray(data)) { if (!s().formatterTemplates) s().formatterTemplates = []; s().formatterTemplates.push(...data); upFTemplates(); saveSettingsDebounced(); toastr.success(`已导入 ${data.length} 个模板`); } } catch (e) { toastr.error('导入失败: ' + e.message); } }; inp.click(); });
        ...
        document.getElementById('mm_llm_save_p').addEventListener('click', () => {
            ...
            refreshAllLlmPresetSelects(); document.getElementById('mm_llm_presets').value = s().llmPresets.length - 1; saveSettingsDebounced(); toastr.success('已保存');
        });
        document.getElementById('mm_llm_upd_p').addEventListener('click', () => {
            const i = Number(document.getElementById('mm_llm_presets').value); if (i < 0 || i >= s().llmPresets.length) return;
            const modelSel = document.getElementById('mm_llm_model_sel');
            const model = modelSel.style.display !== 'none' ? modelSel.value : document.getElementById('mm_llm_model').value;
            ...
            refreshAllLlmPresetSelects(); document.getElementById('mm_llm_presets').value = i; saveSettingsDebounced(); toastr.success('已更新');
        });
        document.getElementById('mm_llm_del_p').addEventListener('click', () => {
            ...
            else { ['mm_llm_url', 'mm_llm_key', 'mm_llm_model'].forEach(id => document.getElementById(id).value = ''); document.getElementById('mm_llm_format').value = API_FORMATS.OAI; document.getElementById('mm_llm_model').style.display = ''; document.getElementById('mm_llm_model_sel').style.display ='none'; }
            saveSettingsDebounced();
        });
        document.getElementById('mm_llm_fetch').addEventListener('click', async () => {
            ...
            try {
                ...
                if (m.length) { const sel = document.getElementById('mm_llm_model_sel'); sel.innerHTML = ''; sel.style.display = ''; document.getElementById('mm_llm_model').style.display = 'none'; m.forEach(it => { const o = document.createElement('option'); o.value = it; o.textContent = it; sel.appendChild(o); }); sel.value = m[0]; toastr.success('获取成功'); }
        ...
        document.getElementById('mm_llm_test_conn').addEventListener('click', async () => {
            ...
            const model = modelSel.style.display !== 'none' ? modelSel.value : document.getElementById('mm_llm_model').value;
            try {
                if (format === API_FORMATS.OAI) { await proxyFetch(normalizeOaiUrl(url), { method: 'POST', headers: { 'Content-Type': 'application/json', ...(key ? { 'Authorization': `Bearer ${key}` } : {}) }, body: { model, messages: [{ role: 'user', content: 'Say connected' }], temperature: 0.1 } }); }
                else { const gUrl = `${url.replace(/\/+$/, '')}/v1beta/models/${model}:generateContent`; await proxyFetch(gUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(key ? { 'x-goog-api-key': key } : {}) }, body: { contents: [{ role: 'user', parts: [{ text: 'Say connected' }] }] } }); }
                toastr.success('API连通成功！');
            } catch (e) { toastr.error(e.message); }
        });
...
function createUi() {
    ...
    $('#mm_wand_item').on('click', () => { const menu = document.getElementById('extensionsMenu'); if (menu) menu.style.display = 'none'; openConfigPanel(); });
    loadSettings();
...
jQuery(async () => {
    ...
    eventSource.on(event_types.CHARACTER_MESSAGE_RENDERED, async (id) => {
        ...
        } else { extractSegmentsOnly(id); }
        injectBubbles(id);
    });
    eventSource.on(event_types.CHAT_CHANGED, () => { setTimeout(refreshAllBubbles, 600); });

    $(document).on('mousedown touchstart', '.mes_quote_tts', function () {
        const id = Number($(this).closest('.mes').attr('mesid')); longP = false;
        timer = setTimeout(async () => {
            longP = true;
            if (s().formatterEnabled) { toastr.info('生成结构中...'); if (await generateMessageSpeech(id, true)) toastr.success('生成成功！'); }
    ...
    $(document).on('click', '.mes_quote_tts', function (e) {
        ...
        clickTimer = setTimeout(async () => {
            ...
            if (s().formatterEnabled) { if (!s().serverHistory[key]?.versions?.length) return toastr.warning('请先长按。'); playGeneratedMessage(id); }
    ...
    $(document).on('click', '.mm-bubble', async function () {
        ...
        const mesid = Number($(this).data('mesid')), segidx = Number($(this).data('segidx'));
        const { key } = getMessageData(mesid);
        const h = s().serverHistory[key];
        const item = h?.versions?.[h.activeIndex]?.items?.[segidx]; if (!item) return;
        const $b = $(this); $b.addClass('mm-bubble-loading');
        try {
            const blob = await getAudioBlob(item);
            $b.removeClass('mm-bubble-loading').addClass('mm-bubble-playing');
            refreshBubbleStates(mesid); refreshAllMessageButtons();
            const url = URL.createObjectURL(blob);
            ...
            await audio.play();
        } catch (e) { $b.removeClass('mm-bubble-loading mm-bubble-playing'); toastr.error('播放失败: ' + e.message); }
    });
});
