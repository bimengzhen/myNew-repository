...
const API_HOST_OPTIONS = [
    ...
    { value: 'https://api.minimax.io',label: '国际源 (api.minimax.io)' },{ value: 'https://api.minimaxi.chat',label: '备用源 (api.minimaxi.chat)' },{ value: 'custom',                   label: '🔗 自定义中转站' },
];
...
async function loadAllWbEntries() {
    ...
    for (const [worldName, source] of worldMap) {
        ...
        } catch (e) { console.warn('[MiniMax] 加载世界书 "' + worldName + '" 失败:', e.message); }
    }
    return allEntries;
}
...
async function directMinimaxTts(text, options) {
    ...
    // emotion 放在 voice_setting 里
    if (emotion) body.voice_setting.emotion = emotion;

    // language_boost 放在顶层（和 curl 示例一致）
    ...
    if (!res.ok) {
        ...
        try {
            ...
            msg = e.base_resp?.status_msg || e.error?.message || e.message || e.detail || JSON.stringify(e).slice(0, 200) || msg;
        } catch (_) {
    ...
    console.log('[MiniMax TTS] 响应字段:', Object.keys(data), data.data ? Object.keys(data.data) : '');
    ...
    // 优先处理 URL 型响应（中转站常见）
    const audioUrl = data.data?.audio_url || data.audio_url || data.data?.url || data.url || data.data?.audio_file || data.audio_file;
    ...
    throw new Error('API 返回格式无法识别，请检查控制台日志');
}
...
async function getAudioBlob(item) {
    ...
    try {
        const res = await fetch(PROXY_ENDPOINT, {
            ...
            body: JSON.stringify({
                text: item.text, apiHost: s().apiHost, model: item.options.model, voiceId: item.options.voiceId,
...
function makeBubble(mesid, segidx, item, withText = false) {
    ...
    el.innerHTML = withText ? `<i class="fa-solid fa-volume-low"></i>${escHtml(item.text.length > 20 ? item.text.slice(0, 20) + '' : item.text)}` : `<i class="fa-solid fa-volume-low"></i>`;
...
function injectBubbles(mesid) {
    ...
    items.forEach(({ it: item, idx }) => {
        ...
        if (isQuoteMode) { if (!injectAfterClosingQuote(textEl, item.text, bubble)) unmatched.push({ item, idx }); }
        else { if (!injectAfterText(textEl, item.text, bubble)) unmatched.push({ item, idx }); }
    });
...
function refreshAllBubbles() {
    if (!s().showBubbles) return;
    document.querySelectorAll('#chat .mes[mesid]').forEach(el => { const id = Number(el.getAttribute('mesid')); extractSegmentsOnly(id); injectBubbles(id); });
}
...
.mm-config-close{background:none;border:none;color:inherit;font-size:1.2rem;cursor:pointer;padding:4px 8px;opacity:.6;flex-shrink:0}
...
.mm-tab{background:transparent;border:none;color:inherit;padding:6px 14px;cursor:pointer;border-radius:8px 8px 0 0;font-size:.88rem;opacity:.55;white-space:nowrap}
.mm-tab:hover{opacity:.8;background:rgba(255,255,255,.04)}
.mm-tab.active{opacity:1;background:rgba(255,255,255,.08);font-weight:600}
...
.mm-rule-header-row{display:flex;gap:6px;font-size:.78rem;opacity:.5;padding:0 0 4px}
.mm-rule-header-row>span{flex:1}
.mm-rule-header-row>.mm-rule-toggle{flex:0 0 20px}.mm-rule-header-row>.mm-rule-mode{flex:0 0 68px}.mm-rule-header-row>.mm-rule-del{flex:0 0 30px}
.mm-rule-row{display:flex;gap:6px;margin-bottom:6px;align-items:center}
.mm-rule-row .text_pole{flex:1;min-width:0}.mm-rule-row .mm-rule-toggle{flex:0 0 20px}
.mes_quote_tts{cursor:pointer;opacity:.5}.mes_quote_tts:hover{opacity:.85}.mes_quote_tts.ready{opacity:1;color:#4caf50}
.mm-bubble{display:inline-flex;align-items:center;gap:3px;padding:1px 6px;margin:0 2px;border-radius:10px;background:rgba(110,160,255,.12);cursor:pointer;font-size:.78rem;vertical-align:middle;transition:background .15s;white-space:nowrap}
.mm-bubble:hover{background:rgba(110,160,255,.25)}.mm-bubble i{font-size:.7rem}
...
.minimax-tts-editor-row-flex label{min-width:36px;font-size:.82rem;opacity:.7}
.minimax-tts-editor-actions{display:flex;gap:10px;padding:12px 16px;border-top:1px solid rgba(255,255,255,.08)}
...
function createUi() {
    ...
    refreshAllLlmPresetSelects();eventSource.on(event_types.CHARACTER_SELECTED, () => { if (document.getElementById('mm_b_rows')) renderBindings(); });
...
function renderRules() {
    ...
    rules.forEach((rule, i) => {
        ...
        el.innerHTML = `<input type="checkbox" class="mm-rule-toggle" ${rule.enabled?'checked':''} title="启用/禁用"><input class="text_pole mm-rule-name" placeholder="规则名" value="${escHtml(rule.name||'')}"><input class="text_pole mm-rule-pattern" placeholder="正则 或 /pattern/" value="${escHtml(rule.pattern||'')}"><select class="text_pole mm-rule-mode" style="max-width:68px"><option value="extract" ${rule.mode==='extract'?'selected':''}>提取</option><option value="exclude" ${rule.mode==='exclude'?'selected':''}>排除</option></select><button class="menu_button mm-rule-del" title="删除">×</button>`;
...
<div class="mm-row"><label>语音气泡</label><input id="mm_show_bubbles" type="checkbox"><span class="mm-inline-hint">消息内注入气泡，可单独播放</span></div>
...
<p class="mm-desc">正则规则手动提取朗读内容；或启用副LLM 交由 AI 结构化处理。</p>
...
<p class="mm-desc">发送给副 LLM 之前，对消息文本执行正则处理。</p>
<div class="mm-rule-header-row"><span class="mm-rule-toggle"></span><span>名称</span><span>正则表达式</span><span class="mm-rule-mode">模式</span><span class="mm-rule-del"></span></div>
<div id="mm_pre_rule_rows"></div>
        ...
        ['mm_key','mm_gid','mm_apihost','mm_custom_host','mm_model','mm_voice_sel','mm_voice','mm_speed','mm_vol','mm_tts_lang','mm_autoplay'].forEach(id => {
            ...
            el.addEventListener('input', syncTts); el.addEventListener('change', syncTts);
        });
        ...
        document.getElementById('mm_llm_fetch').addEventListener('click', async () => {
            ...
            try {
                ...
                if (m.length) { const sel = document.getElementById('mm_llm_model_sel'); sel.innerHTML = ''; sel.style.display = ''; document.getElementById('mm_llm_model').style.display = 'none'; m.forEach(it => { const o = document.createElement('option'); o.value = it; o.textContent = it; sel.appendChild(o); }); sel.value = m[0]; toastr.success('获取成功'); }} catch(e) { toastr.error(e.message); }
        });
        document.getElementById('mm_llm_test_conn').addEventListener('click', async () => {
            ...
            try {
                let d;
                if (format === API_FORMATS.OAI) { d = await proxyFetch(normalizeOaiUrl(url), { method: 'POST', headers: { 'Content-Type': 'application/json', ...(key ? { 'Authorization': `Bearer ${key}` } : {}) }, body: { model, messages: [{ role: 'user', content: 'Say connected' }], temperature: 0.1 } }); }
                else { const gUrl = `${url.replace(/\/+$/, '')}/v1beta/models/${model}:generateContent`; d = await proxyFetch(gUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(key ? { 'x-goog-api-key': key } : {}) }, body: { contents: [{ role: 'user', parts: [{ text: 'Say connected' }] }] } }); }
                if (d) toastr.success('API连通成功！');
