...
async function extractAudioFromResponse(d, fmt) {
    ...
    if (a && typeof a === 'string') {
        ...
        return new Blob([ba], { type: mimeMap[fmt] || 'audio/mpeg' });
    }
    // hex型
    ...
    console.error('[MiniMax TTS] 未知响应:', JSON.stringify(d).slice(0, 500));
...
async function directMinimaxTts(text, options) {
    ...
    console.log('[MiniMax TTS] body:', JSON.stringify(body));
    ...
    if (!res.ok) {
        ...
        try {
            ...
            msg = e.base_resp?.status_msg
                ...
                || msg;
        } catch (_) { try { msg = await res.text(); } catch (_) {} }
        throw new Error(msg);
    }
    ...
    if (data.status === 'error' || (data.base_resp?.status_code !== 0 && data.base_resp?.status_code !== undefined)) {
        throw new Error(
            data.base_resp?.status_msg|| (typeof data.error === 'string' ? data.error : data.error?.message)
            || data.message
            || `[${data.code || 'ERROR'}]`
        );
    }
...
async function playNext() {
    ...
    try {
        ...
        activeAudio.src = u;
        activeAudio.onended = () => { URL.revokeObjectURL(u); playNext(); };
        activeAudio.onerror = () => { URL.revokeObjectURL(u); playNext(); };
...
function applyPreProcessRules(text) {
    ...
    for (const rule of rules) {
        let re; try { re = new RegExp(parsePattern(rule.pattern), 'gs'); } catch (_) { continue; }
        if (rule.mode === 'extract') {
            const m = []; let x;
            while ((x = re.exec(text)) !== null) { const t = (x[1] !== undefined ? x[1] : x[0]).trim(); if (t) m.push(t); }
            if (m.length) text = m.join('\n');
...
async function formatWithSecondaryApi(m) {
    const set = s(), preset = (set.llmPresets || [])[set.formatterPresetIdx];
    if (!preset) throw new Error('请先选择LLM预设');
    const fmt = preset.format || API_FORMATS.OAI, prompt = set.formatterSystemPrompt;
    let text;
    if (fmt === API_FORMATS.OAI) {
        const d = await proxyFetch(normalizeOaiUrl(preset.url), {
            method: 'POST',
        ...
        const bu = (preset.url || '').trim().replace(/\/+$/, '');
        const d = await proxyFetch(`${bu}/v1beta/models/${preset.model}:generateContent`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', ...(preset.key ? { 'x-goog-api-key': preset.key.trim() } : {}) },
...
function findCharacterBinding(name) {
    if (!name) return null;
    const set = s(), ctx = getContext(), n = name.toLowerCase();
    const id = ctx.characterId || ctx.character_id || ctx.name2|| 'global';
    return (set.characterBindingsMap[id] || []).find(b => {
        const bn = (b.targetType === TARGET_TYPE.CUSTOM ? b.customName : (b.targetType === TARGET_TYPE.CURRENT_CHARACTER ? ctx.name2 : ctx.name1))?.toLowerCase();
        return bn === n;
    });
}

function buildSynthesisOptions(seg, m) {
    const set = s(), b = findCharacterBinding(seg?.speaker || m?.name);
    return {
        model: seg?.model || b?.model || set.model,
        voiceId: seg?.voiceId || b?.voiceId || set.voiceId,
        speed: Number(seg?.speed ?? set.speed),
        vol: Number(seg?.vol ?? set.vol ?? 1),
...
function runRegexExtraction(cleanText, rules, speaker) {
    ...
    for (const rule of rules.filter(r => r.mode === 'extract')) {
        const fl = (rule.flags || 'g').includes('g') ? (rule.flags || 'g') : (rule.flags || 'g') + 'g';
        let re; try { re = new RegExp(parsePattern(rule.pattern), fl); } catch (_) { continue; }
        let qm;
        while ((qm = re.exec(cleanText)) !== null) {
            const inner = (qm[1]?.replace(/\n+/g, ' ').trim()) || '';
            const full = (qm[0]?.replace(/\n+/g, ' ').trim()) || '';
            if (inner && inner.length >=2) {
                extracted.push({ text: inner, fullMatch: full, speaker });
            }
        }
    }
...
async function generateMessageSpeech(id, forced = false) {
    ...
    try {
        ...
        if (s().formatterEnabled) {
            ...
            const ct = s().ignoreCodeBlocks ? message.mes.replace(/```[\s\S]*?```/g, ' ') : message.mes;
...
function extractSegmentsOnly(id) {
    ...
    const extracted = runRegexExtraction(ct, rules, message.name);
...
async function playGeneratedMessage(id) {
    const { key } = getMessageData(id), h = s().serverHistory[key];
...
function refreshBubbleStates(mesid) {
    const { key } = getMessageData(mesid), h = s().serverHistory[key];
    const items = h?.versions?.[h.activeIndex]?.items || [];
    const el = document.querySelector(`#chat .mes[mesid="${mesid}"]`);
    if (!el) return;
    el.querySelectorAll('.mm-bubble').forEach(b => {
        const it = items[Number(b.dataset.segidx)];
...
function refreshAllMessageButtons() {
    document.querySelectorAll('#chat .mes[mesid]').forEach(el => {
        ...
        if (!message || message.is_system || isHtmlMessage(message.mes)) return;
...
function injectAfterText(container, searchText, insertEl) {
    ...
    while ((node = walker.nextNode())) {
        ...
        const p = node.parentNode, nx = node.nextSibling;
...
function injectBubbles(mesid) {
    ...
    // 先清理旧气泡
    textEl.querySelectorAll('.mm-bubble').forEach(e => e.remove());
    ...
    // 只允许正则提取模式显示气泡
    const isQuoteMode = (s().regexRules || []).some(r => r.enabled && r.mode === 'extract');
    if (!isQuoteMode) return;

    // 只在完整引号后面插入气泡，找不到就不显示
    items.forEach(({ it: item, idx }) => {
        if (!item.fullMatch) return;
        const bubble = makeBubble(mesid, idx, item, false);
        injectAfterText(textEl, item.fullMatch, bubble);
    });

    textEl.dataset.mmBubVer = `${key}:${h?.activeIndex}:${items.length}:q`;
}
...
function removeAllBubbles() {
    document.querySelectorAll('.mm-bubble,.mm-bubble-strip').forEach(e => e.remove());
    document.querySelectorAll('[data-mm-bub-ver]').forEach(e => e.removeAttribute('data-mm-bub-ver'));
}
...
function openParamsEditor(id) {
    ...
    const render = () => {
        ...
        const rows = v.items.map((it, i) => `
            <div class="minimax-tts-editor-item">
                <div style="font-size:0.8rem;opacity:0.6;margin-bottom:4px">说话人:
                    <input class="edit-v" data-prop="speaker" data-idx="${i}" value="${it.speaker || ''}"
                        style="width:100px;height:20px!important;display:inline-block;border:none!important;background:none!important;color:inherit!important;padding:0!important">
                </div>
        ...
        const html = `<div id="minimax_quote_tts_editor" class="minimax-tts-editor-mask"><div class="minimax-tts-editor-dialog">
            <div class="minimax-tts-editor-header">
                ...
                <div style="display:flex;gap:10px;align-items:center;flex:1">
                    ...
                    <button class="menu_button v-del" style="color:#ef5350">删除</button>
                </div>
            ...
            <div class="minimax-tts-editor-actions">
                ...
                <button class="menu_button editor-confirm" style="height:40px">确认选择此版本</button>
            </div>
        </div></div>`;
        ...
        const inv = () => {
            const te = document.querySelector(`#chat .mes[mesid="${id}"] .mes_text`);
            if (te) te.removeAttribute('data-mm-bub-ver');
            setTimeout(() => { injectBubbles(id); refreshBubbleStates(id); refreshAllMessageButtons(); }, 100);
        };

        $('#minimax_quote_tts_editor .editor-close').on('click', () => { $('#minimax_quote_tts_editor').remove(); inv(); });
        $('#minimax_quote_tts_editor .edit-v').on('change input', function () {
            const p = $(this).data('prop'), idx = $(this).data('idx'), val = $(this).val();
            if (p === 'speaker') {
                v.items[idx].speaker = val;
                const b = findCharacterBinding(val);
        ...
        $('#minimax_quote_tts_editor .editor-confirm').on('click', () => { saveSettingsDebounced(); $('#minimax_quote_tts_editor').remove(); refreshAllMessageButtons(); inv(); });$('#minimax_quote_tts_editor select.edit-v').each(function () { $(this).val(v.items[$(this).data('idx')].options.model); });
    };
    render();
}
...
function refreshAllLlmPresetSelects() {
    const ps = s().llmPresets || [];
    const opts = '<option value="-1">--选择预设--</option>' + ps.map((p, i) => `<option value="${i}">${escHtml(p.name)}</option>`).join('');
    document.querySelectorAll('.llm-preset-sel').forEach(el => {
        const c = el.value; el.innerHTML = opts;
...
.mm-voice-lib-row,.mm-binding-row{display:flex;gap:6px;margin-bottom:6px;align-items:center}
.mm-binding-row{flex-wrap:wrap}.mm-binding-row .text_pole{flex:1;min-width:80px}
...
.mes_quote_tts{cursor:pointer;opacity:.5}.mes_quote_tts:hover{opacity:.85}
...
.mm-bubble-loading{opacity:.5;pointer-events:none}
.mm-bubble-playing{background:rgba(255,165,0,.2)}
...
function renderVoiceLibrary() {
    const c = document.getElementById('mm_voice_lib_rows'); if (!c) return; c.innerHTML = '';
...
function refreshVoiceSelects() {
    ...
    document.querySelectorAll('.mm-voice-sel').forEach(el => { const c = el.value; el.innerHTML = opts; if (c && [...el.options].some(o => o.value === c)) el.value = c; });
}

function renderRules() {
    const c = document.getElementById('mm_rule_rows'); if (!c) return; c.innerHTML = '';
    (s().regexRules || []).forEach((r, i) => {
...
function renderPreProcessRules() {
    const c = document.getElementById('mm_pre_rule_rows'); if (!c) return; c.innerHTML = '';
    (s().llmPreProcessRules || []).forEach((r, i) => {
        const el = document.createElement('div'); el.className = 'mm-rule-row';
        el.innerHTML = `<input type="checkbox" class="mm-rule-toggle" ${r.enabled ? 'checked' : ''}><input class="text_pole mm-rule-name" placeholder="规则名" value="${escHtml(r.name || '')}"><input class="text_pole mm-rule-pattern" placeholder="正则" value="${escHtml(r.pattern || '')}"><select class="text_pole mm-rule-mode" style="max-width:68px"><option value="extract" ${r.mode === 'extract' ? 'selected' : ''}>提取</option><option value="exclude" ${r.mode === 'exclude' ? 'selected' : ''}>排除</option></select><button class="menu_button mm-rule-del">×</button>`;
...
function renderBindings() {
    ...
    const ct = $('#mm_b_rows'); if (!ct.length) return; ct.empty();
    const lib = s().voiceLibrary || [];
    s().characterBindingsMap[id].forEach((b, i) => {
        const vo = '<option value="">直接输入</option>' + lib.map(v => `<option value="${escHtml(v.voiceId)}">${escHtml(v.name)}</option>`).join('');
        const lm = lib.find(v => v.voiceId === b.voiceId);
        ...
        row.find('.b-voice-lib').val(lm ? b.voiceId : '').on('change', function () { const v = $(this).val(); b.voiceId = v; row.find('.b-voice').toggle(!v).val(''); saveSettingsDebounced(); });
...
function renderRegexPresets() {
    ...
    (s().regexPresets || []).forEach((p, i) => { const o = document.createElement('option'); o.value = i; o.textContent = p.name; sel.appendChild(o); });
}

function populateConfigFields() {
    const set = s(), el = id => document.getElementById(id);
    if (!el('mm_key')) return;
    ...
    const vs = el('mm_voice_sel'), vi = el('mm_voice');
    const lm = (set.voiceLibrary || []).find(v => v.voiceId === set.voiceId);
...
<div id="mm-config-mask" class="mm-config-mask"><div class="mm-config-dialog">
...
<button class="mm-config-close">✕</button>
</div>
...
<div class="mm-row"><label>API节点</label><select id="mm_apihost" class="text_pole">${API_HOST_OPTIONS.map(o => '<option value="' + o.value + '">' + o.label + '</option>').join('')}</select></div>
...
<p class="mm-desc">为语音ID起名。</p><div id="mm_voice_lib_rows"></div>
...
<p class="mm-desc">为角色分配专属音色。</p><div id="mm_b_rows"></div>
</div>
...
<div id="mm_pre_rule_rows"></div>
        ...
        document.querySelectorAll('#mm-config-mask .mm-tab').forEach(tab => tab.addEventListener('click', function () {
            const t = this.dataset.tab;
            document.querySelectorAll('#mm-config-mask .mm-tab').forEach(b => b.classList.remove('active'));
            this.classList.add('active');
            document.querySelectorAll('#mm-config-mask .mm-tab-panel').forEach(p => p.classList.remove('active'));
            document.querySelector(`#mm-config-mask .mm-tab-panel[data-panel="${t}"]`).classList.add('active');
        ...
        document.querySelector('#mm-config-mask .mm-config-close').addEventListener('click', () => document.getElementById('mm-config-mask').classList.remove('mm-config-open'));

        // TTS配置同步
        const syncTts = () => {
            const vs = document.getElementById('mm_voice_sel');
        ...
        document.getElementById('mm_apihost').addEventListener('change', function () {
            const r = document.getElementById('mm_custom_host_row');
        ...
        // 测试语音
        const TT = { '': '你好，我是MiniMax语音。', zh: '你好，我是MiniMax语音。', en:'Hello, I am MiniMax voice.', ja: 'こんにちは', ko: '안녕하세요' };
        document.getElementById('mm_test_tts').addEventListener('click', async () => {
            try {
                const t = TT[s().ttsLanguage || ''] || TT[''];
                const b = await getAudioBlob({ text: t, options: buildSynthesisOptions(null, null), serverPath: null });
        ...
        document.getElementById('mm_regex_presets').addEventListener('change', function () {
            const i = Number(this.value); if (i < 0 || !s().regexPresets?.[i]) return;
        ...
        document.getElementById('mm_regex_import').addEventListener('click', () => {
            ...
            inp.onchange = async () => { try { const d = JSON.parse(await inp.files[0].text()); if (Array.isArray(d)) { if (!s().regexPresets) s().regexPresets = []; s().regexPresets.push(...d); renderRegexPresets(); saveSettingsDebounced(); toastr.success(`已导入${d.length}个`); } } catch (_) { toastr.error('导入失败'); } };
            inp.click();
        });
        ...
        const upFT = () => {
            ...
            (s().formatterTemplates || []).forEach((t, i) => { const o = document.createElement('option'); o.value = i; o.textContent = t.name; selFT.appendChild(o); });
        };
        ['mm_f_en', 'mm_f_preset_sel', 'mm_f_prompt'].forEach(id => {
            const el = document.getElementById(id); if (el) { el.addEventListener('input', syncFmt); el.addEventListener('change', syncFmt); }
        });
        ...
        document.getElementById('mm_f_upd_t').addEventListener('click', () => {
            ...
            if (i >= 0 && s().formatterTemplates?.[i]) { s().formatterTemplates[i].content = document.getElementById('mm_f_prompt').value; saveSettingsDebounced(); toastr.success('已更新'); }
        });
        document.getElementById('mm_f_del_t').addEventListener('click', () => {
            const i = Number(selFT.value); if (i >= 0) { (s().formatterTemplates || []).splice(i, 1); upFT(); saveSettingsDebounced(); }
        });
        ...
        document.getElementById('mm_f_import_t').addEventListener('click', () => {
            const inp = document.createElement('input'); inp.type = 'file'; inp.accept = '.json';
            inp.onchange = async () => { try { const d = JSON.parse(await inp.files[0].text()); if (Array.isArray(d)) { if (!s().formatterTemplates) s().formatterTemplates = []; s().formatterTemplates.push(...d); upFT(); saveSettingsDebounced(); toastr.success(`已导入${d.length}个`); } } catch (_) { toastr.error('导入失败'); } };
            inp.click();
        });
        ...
        document.getElementById('mm_llm_save_p').addEventListener('click', () => {
            ...
            const ms = document.getElementById('mm_llm_model_sel');
            const m = ms.style.display !== 'none' ? ms.value : document.getElementById('mm_llm_model').value;
            s().llmPresets.push({ name: n, url: document.getElementById('mm_llm_url').value, key: document.getElementById('mm_llm_key').value, format: document.getElementById('mm_llm_format').value, model: m });
            refreshAllLlmPresetSelects(); document.getElementById('mm_llm_presets').value = s().llmPresets.length - 1;
        ...
        document.getElementById('mm_llm_upd_p').addEventListener('click', () => {
            ...
            const ms = document.getElementById('mm_llm_model_sel');
            const m = ms.style.display !== 'none' ? ms.value : document.getElementById('mm_llm_model').value;
            s().llmPresets[i] = { ...s().llmPresets[i], url: document.getElementById('mm_llm_url').value, key: document.getElementById('mm_llm_key').value, format: document.getElementById('mm_llm_format').value, model: m };
        ...
        document.getElementById('mm_llm_del_p').addEventListener('click', () => {
            ...
            refreshAllLlmPresetSelects();
            const ni = Number(document.getElementById('mm_llm_presets').value);
        ...
        document.getElementById('mm_llm_fetch').addEventListener('click', async () => {
            const u = document.getElementById('mm_llm_url').value.trim().replace(/\/chat\/completions$/, '').replace(/\/+$/, '');
            const k = document.getElementById('mm_llm_key').value.trim();
            const f = document.getElementById('mm_llm_format').value;
            try {
                let m = [];
                if (f === API_FORMATS.OAI) { const d = await proxyFetch(`${u}/models`, { headers: k ? { 'Authorization': `Bearer ${k}` } : {} }); m = d.data?.map(x => typeof x === 'string' ? x : x.id) || []; }
                else { const d = await proxyFetch(`${u}/v1beta/models`, { headers: k ? { 'x-goog-api-key': k } : {} }); m = d.models?.map(x => x.name.replace('models/', '')) || []; }
                if (m.length) {
        ...
        document.getElementById('mm_llm_test_conn').addEventListener('click', async () => {
            const u = document.getElementById('mm_llm_url').value.trim();
            const k = document.getElementById('mm_llm_key').value.trim();
            const f = document.getElementById('mm_llm_format').value;
            const ms = document.getElementById('mm_llm_model_sel');
            const m = ms.style.display !== 'none' ? ms.value : document.getElementById('mm_llm_model').value;
            try {
                if (f === API_FORMATS.OAI) await proxyFetch(normalizeOaiUrl(u), { method: 'POST', headers: { 'Content-Type': 'application/json', ...(k ? { 'Authorization': `Bearer ${k}` } : {}) }, body: { model: m, messages: [{ role: 'user', content: 'Hi' }], temperature: 0.1 } });
                else await proxyFetch(`${u.replace(/\/+$/, '')}/v1beta/models/${m}:generateContent`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(k ? { 'x-goog-api-key': k } : {}) }, body: { contents: [{ role: 'user', parts: [{ text: 'Hi' }] }] } });
                toastr.success('连通成功！');
            } catch (e) { toastr.error(e.message); }
        });
...
function createUi() {
    injectStyles();
    $('#extensionsMenu').append('<div id="mm_wand_item" class="list-group-item flex-container flexGap5" title="MiniMax TTS"><div class="fa-solid fa-volume-high extensionsMenuExtensionButton"></div>MiniMax语音</div>');
    $('#mm_wand_item').on('click', () => {
        const m = document.getElementById('extensionsMenu'); if (m) m.style.display = 'none';
        openConfigPanel();
    });
    loadSettings();
    syncToSTSecrets((s().apiKey || '').trim(), (s().groupId || '').trim());
...
jQuery(async () => {
    ...
    $(document).on('click', '.mes_quote_tts', function (e) {
        ...
        clickTimer = setTimeout(async () => {
            clickTimer = null;
            const id = Number($(e.target).closest('.mes').attr('mesid'));
            const { key } = getMessageData(id);
            if (s().formatterEnabled) {
                if (!s().serverHistory[key]?.versions?.length) return toastr.warning('请先长按');
    ...
    $(document).on('click', '.mm-bubble', async function () {
        ...
        try {
            ...
            const a = new Audio(u);
            a.onended = () => { URL.revokeObjectURL(u); $b.removeClass('mm-bubble-playing'); };
            a.onerror = () => { URL.revokeObjectURL(u); $b.removeClass('mm-bubble-playing'); };
            await a.play();
        } catch (e) {
