...
function openParamsEditor(id) {
    ...
    const render = () => {
        ...
        $('#minimax_quote_tts_editor .editor-confirm').on('click', () => { saveSettingsDebounced(); $('#minimax_quote_tts_editor').remove(); refreshAllMessageButtons(); invalidateBubbles(); });$('#minimax_quote_tts_editor select.edit-v').each(function(){ $(this).val(v.items[$(this).data('idx')].options.model); });}; render();
}
  ...
  <div id="mm-config-dialog" class="mm-config-dialog">
    ...
    <div class="mm-config-body">
      ...
      <div class="mm-tab-panel active" data-panel="tts">
        ...
        <div class="mm-row" id="mm_custom_host_row" style="display:none"><label>中转地址</label><input id="mm_custom_host" class="text_pole" type="text" placeholder="https://xxx.com/v1/t2a_v2（完整请求路径）"></div>
        ...
        <div class="mm-row"><label>语言</label><select id="mm_tts_lang" class="text_pole">
        ...
        <div class="mm-row"><label>语音气泡</label><input id="mm_show_bubbles" type="checkbox"><span class="mm-inline-hint">在消息下方注入气泡，点击气泡单独收听该段语音</span></div><div class="mm-row"><button id="mm_test_tts" class="menu_button"><i class="fa-solid fa-play"></i> 测试语音</button></div><div class="mm-section-title">语音库 <button id="mm_add_voice" class="menu_button" style="font-size:0.8rem;padding:3px 10px">+ 添加</button></div>
      ...
      <div class="mm-tab-panel" data-panel="format">
        <p class="mm-desc">两种格式化方式：正则规则手动提取朗读内容；启用副LLM 后交由 AI 结构化处理（支持多角色/情感），两者互斥。</p>
        ...
        <p class="mm-desc">发送给副 LLM 之前，对消息文本执行正则处理（如剔除 &lt;think&gt; 思维链标签）。</p>
        <div class="mm-rule-header-row">
          <span class="mm-rule-toggle"></span>
          <span>名称</span>
          <span>正则表达式</span>
...
jQuery(async () => {
    ...
    $(document).on('mousedown touchstart', '.mes_quote_tts', function(){
        ...
        timer = setTimeout(async () => {
            ...
            } else { if (await generateMessageSpeech(id, true)) playGeneratedMessage(id); }}, 600);
